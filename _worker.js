export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith("/v1/data/")) {
      ...
      // 🔒 セキュリティガード（簡易SSRF対策の強化）
      const host = actualTargetUrl.hostname.toLowerCase();
      const isPrivateIP = 
        host === "localhost" || 
        host === "127.0.0.1" || 
        host === "[::1]" ||
      ...
      if (actualTargetUrl.host === url.host || isPrivateIP) {
        return new Response("Access Denied: Invalid Destination", { status: 403 });
      }
      ...
      try {
        ...
        // 🍪 複数クッキーに対応する安全な置換 (getSetCookieの導入)
        ...
        // HTML の場合の書き換え（HTMLRewriter によるストリーム処理）
